const socket = new WebSocket("/wsSinglePlayer");
...
function draw() {
    ...
    const now = new Date();
    if (cronometro && !total_time) {
        const elapsed_time = now - cronometro;
        timer_span.innerText =
            pad_left(Math.floor(elapsed_time / 1000 / 60)) +
            ":" +
            pad_left(Math.floor(elapsed_time / 1000) % 60);
    }
    ...
    switch (phase) {
        case phases.GetUpdates:
            ...
            arc(
                ...
                R - 20,
                R - 20,
                map(now - timer_start, 0, time, 0, TWO_PI) - PI / 2,
                -PI / 2,
            );
            break;
        case phases.Won:
...
function get_updates(data_view) {
    ...
    switch (type) {
        case 0:
            ...
            if (gameover) {
                total_time = new Date() - cronometro;
                cronometro = null;
